...
%
 ...
 % patchCube = @(X,w) ...
  ...
  % kernel 
    ...
    numberOfBlocks = ceil( (Nthreads*Nthreads)/prod(threadsPerBlock) );
  ...
  %Data 
      ...
      tic;
      D = gather( feval(kernel, single(imgGPU) , single(gaussian), prod(patchSize) ,filtSigma , Nthreads, single(D)) );
      toc
  ...
  % reshape for image
  If = reshape( D, [Nthreads Nthreads] );

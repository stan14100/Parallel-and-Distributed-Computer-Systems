...
// compile C programm with gcc -fPIC -pthread -shared -o concBP.so BP_wPthreads.c
     ...
     , float *_data, int *_res,int batchsize, int datadim, int resdim, float eta)
{
     ...
     threads = (pthread_t *)malloc(Ncores*sizeof(pthread_t));
     bpd = (BPdata *)malloc(Ncores*sizeof(BPdata));

     int chunksize = floor((double)((batchsize+1)/Ncores)) ;
     int startData, endData;

     for (i=0 ; i<Ncores; i++)
     {
          bpd[i].datadim = datadim;
          bpd[i].resdim = resdim;
          bpd[i].Nlayers = Nlayers;
          bpd[i].sizes = sizes;
          bpd[i].weights = _weights;
          bpd[i].biases = _biases;
          ...
          bpd[i].delta_nabla_b = (float **)malloc(sizeof(float *) * (bpd[i].Nlayers-1));
          bpd[i].delta_nabla_w = (float **)malloc(sizeof(float *) * (bpd[i].Nlayers-1));
          for (j=0 ; j<bpd[i].Nlayers-1 ; j++)
          {
               bpd[i].delta_nabla_b[j] = (float *)malloc(sizeof(float)*bpd[i].sizes[j+1]);
               //allocated.
               bpd[i].delta_nabla_w[j] = (float *)malloc(sizeof(float)*(bpd[i].sizes[j+1]*bpd[i].sizes[j]) );
               //allocated
          }
          ...
          if (i != Ncores-1)
          {
               bpd[i].startData = i*chunksize;
               bpd[i].endData = (i+1)*(chunksize)-1;
          }else //the last thread takes all the remaining data
          {
               bpd[i].startData = i*chunksize;
               bpd[i].endData = batchsize-1;
          }
          pthread_create(&threads[i], &joinable, callBP, (void *)(bpd+i) );
     }
     //wait fot threads to finish
     for (i=0; i<Ncores ; i++)
          pthread_join(threads[i],NULL);

     //threads finished. data calculated. let's proceed
     ...
     for (i=0 ; i<Nlayers-1 ; i++)
     {
          ...
          for (j=0 ; j<sizes[i+1] ; j++)
          {
               //initialize
               nabla_b(i, j) = 0;
               for (l=0; l<Ncores; l++) //compute : is the sum
                    nabla_b(i, j) += bpd[l].delta_nabla_b[i][j];
          }
          ...
          for (j=0; j<sizes[i+1] ; j++)
          {
               for (k=0 ; k <sizes[i] ; k++)
               {
                    ...
                    nabla_w(i,j,k) = 0;
                    for (l=0; l<Ncores; l++)
                         nabla_w(i,j,k) += bpd[l].delta_nabla_w[i][k+sizes[i]*j];
               }
          }
     }
...
//     printf("Time per mini batch (serial) : %fsec\n", hash_time);
